// Hosted AI-hint endpoint (Vercel serverless function). Uses the same shared
// engine as the local server; the Gemini key comes from a Vercel env var, so it
// stays server-side and is never sent to the browser.
const { getCredentialError, validateRequestState, requestValidatedPath, provider, model } = require("../lib/hint-engine");
...
module.exports = async (req, res) => {
  ...
  const stateError = validateRequestState(body);
  if (stateError) {
  ...
  try {
    const logError = (action, details) => console.warn("[hint]", action, JSON.stringify(details));
    const result = await requestValidatedPath(body, logError);
    res.status(200).json({ ...result, latency_ms: Date.now() - startedAt, provider, model });
  } catch (error) {
