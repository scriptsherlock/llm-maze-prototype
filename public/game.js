...
// The opening screen of the whole study, then -- after the first questionnaire -- a
// short brief on what the mazes are. The maze scene, the controls and the counters are
// all hidden until the practice starts: none of it means anything yet, and a Moves
// counter ticking behind a welcome screen just looks broken.
function setChromeVisible(visible) {
  document.querySelector(".experiment-controls")?.classList.toggle("hidden", !visible);
  document.querySelector(".participant-shell .stats-row")?.classList.toggle("hidden", !visible);
}
...
function showIntro() {
  ...
  document.getElementById("helpButton")?.classList.add("hidden");
  if (link) link.classList.add("hidden");     // questionnaires are framed, not linked
  setChromeVisible(false);

  if (introStep === 0) {
    title.textContent = "Welcome";
    body.textContent = "Thank you for taking part. In this session you will answer a few short "
      + "questionnaires and complete a series of maze tasks. Please work through it in one "
      + "sitting, and do not refresh the page.";
    button.textContent = "Continue";
    logState("intro_welcome_shown");
    return;
  }

  title.textContent = "What happens next";
  const parts = ["The next step is a short task: navigating through mazes to find an exit. "
    + "A training phase will start first, to help you understand the controls."];
  // Only the groups that actually get an assistant are told about one. Saying this to
  // the control group would describe something they never see.
  if (aiCondition) {
    parts.push("In the mazes you will see hints on your screen, generated by AI, to help you "
      + "navigate. There may be more than one hint. You are free to follow them or to find your "
      + "own path. The goal is to find the exit.");
  }
  body.innerHTML = parts.map((t) => `<span class="intro-para">${t}</span>`).join("");
  button.textContent = "Start the training";
  logState("intro_briefing_shown", { mentions_ai: aiCondition });
}

// Welcome -> questionnaire -> brief -> practice.
function advanceIntro() {
  if (introStep === 0) {
    ...
    if (!url) { thenBrief(); return; }
    document.getElementById("introScreen")?.classList.add("hidden");
    introOpen = false;
    openSurveyOverlay(withRunParams(url), "Questionnaire 1 of 3", thenBrief);
    return;
  }
