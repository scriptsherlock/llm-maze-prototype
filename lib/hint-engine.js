...
// Waits of 20s then 40s: enough for a per-minute window to roll over.
...
async function callProvider(payload, url, apiKey, label, timeoutMs = hintTimeoutMs, retriesLeft = RATE_LIMIT_RETRIES) {
  ...
  try {
    ...
    if (response.status === 429 && retriesLeft > 0) {
      const waitMs = RATE_LIMIT_BACKOFF_MS * (RATE_LIMIT_RETRIES - retriesLeft + 1);
...
function buildSolutionsPayload(state, count) {
  ...
  if (provider === "gemini") {
    ...
    return { model, temperature: 0.6, max_completion_tokens: 65536, reasoning_effort: "high",
      messages: [{ role: "system", content: instructions }, { role: "user", content: input }],
