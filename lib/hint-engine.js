...
async function requestValidatedPath(state, logError = () => {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (firstAttemptPath === null && Array.isArray(parsed && parsed.full_path) && parsed.full_path.length > 0) {
      firstAttemptPath = parsed.full_path.map(normalizeCell);
    }
...
function buildGeminiPayload(state, attempt, previousError) {
  return {
    model,
    temperature: 0.4,
    // Raised alongside reasoning_effort: high reasoning consumes completion
    // tokens, so keep enough headroom for the full JSON path not to be truncated.
