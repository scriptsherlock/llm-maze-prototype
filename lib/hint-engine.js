...
function routeEvalInstructions() {
  return [
    "You are the AI navigation assistant in a hedge-maze study.",
    "The player is at a junction. For EACH candidate next cell in `branches`, judge how good going that way is for reaching the goal.",
    "Coordinates use x for column and y for row. 0 means open, 1 means wall.",
    "Use open_cell_graph as the authority for which moves are legal.",
    "For each branch give `steps` (whole grid steps to the goal that way), a `verdict`",
    "(toward_goal = a good short way; detour = reachable but noticeably longer; dead_end = leads nowhere useful),",
    "and a short `reason` (max ~10 words) — especially WHY a detour or dead_end fails,",
    "e.g. 'dead-ends after two cells', 'loops back to here', 'the long way around'.",
    "Return exactly one entry per branch, in the same order, echoing that branch's x and y.",
  ].join(" ");
}
...
function batchInstructions() {
  return [
    "You are the AI navigation assistant in a hedge-maze study.",
    "For EVERY junction in `junctions`, evaluate each of its branch cells for reaching the goal.",
    "Coordinates use x for column and y for row. 0 means open, 1 means wall. Use open_cell_graph for legal moves.",
    "For each branch give `steps` (whole grid steps to the goal that way), a `verdict`",
    "(toward_goal = good short way; detour = reachable but noticeably longer; dead_end = leads nowhere useful),",
    "and a short `reason` (max ~10 words) — especially WHY a detour or dead_end fails.",
    "Echo each junction's x,y and each branch's x,y, in the same order.",
  ].join(" ");
}
