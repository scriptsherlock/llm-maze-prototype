...
function junctionTraceInstructions() {
  return [
    "ROLE",
    ...
    "- reaches_goal=false (a DEAD END): you MUST PROVE it. In `path`, walk from the branch cell into the pocket until you can go no further, ending at the dead-end cell. This is checked; an unproven dead end is rejected. NEVER claim a dead end you cannot walk to the end of.",
    "- reaches_goal=true (it reaches the exit): give your best `steps_estimate` (whole steps from the junction to the goal that way). `path` is optional here — include the route if you can trace it, else leave it empty.",
    "",
    ...
    "- If reaches_goal=false the path MUST end at a true dead-end (a cell whose only open neighbour is the one you came from).",
    "",
...
function junctionTraceSchema() {
  return {
    ...
    properties: { branches: { type: "array", items: {
      type: "object", additionalProperties: false, required: ["x", "y", "reaches_goal", "steps_estimate", "path", "reason"],
      properties: { x: { type: "integer" }, y: { type: "integer" }, reaches_goal: { type: "boolean" }, steps_estimate: { type: "integer" }, path: cellListSchema(), reason: { type: "string" } },
    } } },
  };
}
...
function buildJunctionTracePayload(state, feedback) {
  const instructions = junctionTraceInstructions();
...
async function evaluateJunctionTraced(state, logError = () => {}, maxAttempts = 4) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    let parsed;
    try {
      parsed = parseLlmResponse(await callLlm(buildJunctionTracePayload(state, feedback), batchTimeoutMs), logError);
    } catch (error) {
    ...
    for (const b of state.branches) {
      ...
      if (!mb.reaches_goal) {
        // DEAD END: only accepted with a valid path that really ends at a dead end.
        // An unproven dead end is the damaging error, so we reject and re-ask.
        if (!v.valid) { bad.push(`${b.x},${b.y}: dead-end claim needs a valid path (${v.why})`); continue; }
        if (!v.endIsDeadEnd) { bad.push(`${b.x},${b.y}: reaches_goal=false but the path does not end at a dead-end`); continue; }
        results.push({ branch: b, reaches: false, steps: v.steps, reason, proven: true });
        continue;
      }
    ...
    feedback = `Some branches were rejected. Fix ONLY these, obeying the path rules exactly: ${bad.join("; ")}. If you cannot walk a branch to a true dead end, it is NOT a dead end — set reaches_goal=true instead.`;
