...
function solutionsInstructions(count) {
  return [
    "ROLE",
    "You are solving a hedge maze. This maze HAS LOOPS, so there are SEVERAL genuinely different ways from the start to the exit — not one.",
    "",
    "INPUT (JSON): `maze` rows ('0'=open, '1'=wall; cell (x,y)=maze[y][x]), `start`, `goal`, and `open_cell_graph` (each open cell -> its open neighbours = the ONLY legal moves).",
    "",
    `TASK: find up to ${count} DIFFERENT routes from start to goal.`,
    "- Every route starts EXACTLY at `start` and ends EXACTLY at `goal`.",
    "- Consecutive cells must differ by 1 in x or y and both be open — an edge in open_cell_graph. Never cross a wall, never jump.",
    "- Never revisit a cell within a route.",
    "- Routes must be GENUINELY different: each one must use at least one passage the others do not. Take a different fork somewhere and go round the other side of a loop. Do not return the same route twice.",
    "- Give the shortest route you can find first, then the alternatives.",
    "",
    "Return only JSON matching the schema. Each route also gets a short `note` (<=10 words) saying how it differs.",
  ].join("\n");
}
...
function solutionsSchema() {
  return {
    ...
    properties: { routes: { type: "array", items: {
      ...
      properties: { path: cellListSchema(), note: { type: "string" } },
    } } },
  };
}

function buildSolutionsPayload(state, count) {
  const instructions = solutionsInstructions(count);
  ...
  if (provider === "gemini") {
    return { model, temperature: 0.6, max_completion_tokens: 32768, reasoning_effort: "medium",
      messages: [{ role: "system", content: instructions }, { role: "user", content: input }],
...
async function findMazeSolutions(state, count = 6, logError = () => {}, maxAttempts = 3) {
  ...
  for (let attempt = 0; attempt < maxAttempts && found.length < count; attempt += 1) {
    ...
    for (const r of (parsed && parsed.routes) || []) {
      const v = validateSolution(r.path || [], state);
      ...
      signatures.add(sig);
      found.push({ steps: v.steps, note: String(r.note || ""), path: v.cells });
    }
  }
